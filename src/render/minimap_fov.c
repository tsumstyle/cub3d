...
/*   Updated: 2025/05/09 12:41:44 by bbierman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_minimap_ray(t_data *game, double rel_angle)
{
	...
	ray_dir_x = cos(dir_angle);
	ray_dir_y = sin(dir_angle);
	len = calculate_wall_distance(game, dir_angle, &game->player.orientation, true);
	if (len > MINI_RAY_LENGTH)
		len = MINI_RAY_LENGTH;
	double	start_x = MAP_OFFSET_X + game->player.x * MINI_TILE_SIZE;
	double	start_y = MAP_OFFSET_Y + game->player.y * MINI_TILE_SIZE;
	while (i < len * MINI_TILE_SIZE)
	{
		px = start_x + ray_dir_x * i;
		py = start_y + ray_dir_y * i;
